...
void Model::back_propagate() {
    Matrix* A_prev = layers[layers.size()-1].get_activation();
    Matrix dAL = Loss::mean_square_prime(Y, *A_prev);
    auto dA = &dAL;
    for (int i = layers.size()-1; i >= 0; i--) {
        auto& layer = this->layers[i];
        dA = layer.back_propagate(*A_prev, *dA);
        A_prev = layers[i].get_activation();
    }
}
